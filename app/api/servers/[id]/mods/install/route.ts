...
export async function POST(
  ...
  try {
    ...
    // Download the mod
    const modResponse = await axios.get(url, {
      responseType: 'arraybuffer'
    })

    const modBuffer = Buffer.from(modResponse.data)

    // Upload to Pterodactyl server
    await axios.post(
      `${settings.panelUrl}/api/client/servers/${server.pterodactylIdentifier}/files/write?file=${encodeURIComponent(`/mods/${fileName}`)}`,
      modBuffer,
      {
        headers: {
