...
export async function POST(
  ...
  try {
    ...
    // Unsuspend on Pterodactyl if ID exists
    if (server.pterodactylId) {
      const success = await pterodactylService.unsuspendServer(server.pterodactylId)
      if (!success) {
        return NextResponse.json({ error: 'Failed to unsuspend on Pterodactyl' }, { status: 500 })
      }
    }
